...
void play_game(que_bank question){
    ...
    while(time_left>0){
        if(_kbhit()){// iske liye coni.h file include ki hai , ye check karega keyboard hit hua ya nhi..
            ans = getch(); // ye bina enter press kiye hi input le leta hai
            ans =  toupper(ans);
            printf("%c\n",ans);
...
void kbcc(){
    que_bank* question;
    int no_que = read_que("B24CH1028_B24CS1076_B24CH1049_B24CS1049_B24CM1062_KBCCquebank.txt",&question);
    printf("\t\t\t\t%sChalo khelte hai KON BANEGA CRORPATI IN C urf KBCC!!!\n%s",Pink,color_end);
    for(int i =0; i<no_que; i++){
        print_que((question)[i]);
    }
